...
| --- | --- | --- |
| `cv_pipeline_depth` | `1, 2, 3, 4` | `CVPipeliningOptions.setDepthInUnrollMode` |
| `cv_num_buffers` | `1..cv_pipeline_depth` | a new independent CVPipeline/multibuffer option |
| `vf_merge_level` | `0, 1, 2` | existing `NPUOptions.vf_merge_level` and `--enable-vf-merge-level` |

This triangular search space has 30 requested combinations. Do not silently
coerce or drop a combination. Record it as `unsupported`, `compile_failed`,
`ub_overflow`, `incorrect`, or `measured` with a diagnostic.

Every accepted operator case must therefore produce 30 rows. A failed or
...
- `vf_merge_level` is already passed to `bishengir-compile`. Clean up the
  duplicate default declaration in `NPUOptions` and add explicit validation
  for `{0, 1, 2}` before using it as an experiment axis.
- `cv_pipeline_depth` and `cv_num_buffers` are not independent today.
  `CVPipelineImpl` receives `setDepthInUnrollMode` as `numMultibuffer`, so an
  explicit depth also determines the buffer count. When the value is `-1`, it
  may instead infer the count from `annotation.mark` multibuffer metadata.
  Separate scheduling depth from physical buffer multiplicity before exposing
  the requested Cartesian/triangular search space.
- Depth `1` currently makes `CVPipeliningPass` return without pipelining, while
  the implementation normally requires at least two buffers. Define and test
  the semantics of `(depth > 1, numBuf = 1)`; if safe overlap is impossible,
  report those candidates as unsupported instead of manufacturing a result.
- The backend initializes `required_ub_bits`, parses `UB size = <N> bits` from
  compiler output, and the existing UB tuner can read the value. Per-config
  autotune reporting does not yet retain it reliably, and a value of zero must
  mean "measurement missing", not "zero UB".
...
### 3. Plumb two genuinely independent CV controls

Add validated fields to `NPUOptions` for `cv_pipeline_depth` and
`cv_num_buffers`, propagate them through `third_party/ascend/backend/compiler.py`
to explicit `bishengir-compile` options, then into `HIVMPipelineOptions` and
`CVPipeliningOptions`.

Refactor CVPipeline internals so that:

- depth controls schedule/unroll distance and work-item overlap;
- numBuf controls buffer version count and memory expansion;
- automatic mark inference is used only when numBuf is not explicitly set;
- UB prediction consumes the same resolved pair as the production pass;
- invalid relationships fail with a stable diagnostic;
- neither field changes the TTIR or the meaning of `vf_merge_level`.

Add focused IR tests for equal and unequal pairs, especially `(1,1)`, `(2,1)`,
`(2,2)`, `(3,2)`, `(3,3)`, `(4,2)`, and `(4,4)`.

### 4. Extend autotune configuration and result capture

Generate `triton.Config` candidates for the 30 requested triples and pass the
...
- resolved depth, resolved buffer count, and VF merge level;
...
4. **Compile-only gate:** enumerate all 30 triples and classify every result,
...
6. **Corpus gate:** dynamically classify sensitivity for each copied candidate,
   ...
   or deferred source.
7. **Generalization gate:** complete the 30-row table for multiple accepted
...
autotune works. Completion requires independent control of all three requested
variables, per-candidate UB and latency records, correctness filtering, and a
reproducible complete table for every accepted operator. No winning
configuration is part of the requested deliverable.
