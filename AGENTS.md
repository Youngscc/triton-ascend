...
- Experiment branch: local `codex/experiment-main-dev`, based on
  `upstream/main-dev`
- Triton-Ascend source baseline: `8516a565eb4e95cf01dfb1a0279d5a2328914abb`
- AscendNPU-IR compatibility baseline: `aea934a66646e837c54fea11e87db54d42eb3221`
...
| --- | --- | --- |
| A3: `depth`; A5: DynamicCV state plus `buf_slot_num_of_veccore` | A3 `1, 2, 3, 4`; A5 `off`, then `1, 2, 3, 4` | A3 uses native static CV workspace/depth with DynamicCV disabled; A5 always disables static CVPipeline, first runs with DynamicCV disabled, then enables DynamicCV and varies its Vector Core buffer slots while fixing cross-core/GM slots to 1 |
...
- On A3, CV scheduling depth and workspace-buffer count use BishengIR's native
  ...
  disables both static CVPipeline and DynamicCV; numeric first-axis values
...
- on A5, static CVPipeline is always disabled; `off` also disables DynamicCV, while numeric `buf_slot_num_of_veccore` values enable DynamicCV and cross-core/GM slot counts stay fixed at 1;
